...
fun DFS(g: Grafo, inicio: Int): List<Int>  { // presenta el recorrido DFS del grafo g desde el nodo inicio
    val visitado = mutableListOf<Int>()
    //Creamos una pila para almacenar los vecinos de vertices visitados
    val pila = ArrayDeque<Int>()
    //Agregamos el vertice inicial a la lista de visitado y a la pila para buscar sus vecinos
    visitado.add(inicio)
    pila.add(inicio)
    ...
    while (pila.isNotEmpty()){
        ...
        val vertice_actual = pila.removeLast()
        //Iteramos sobre los vecinos del vertice actual
        for (i in getVecinos(g, vertice_actual)){
            //Si alguno de los vecinos no ha sido visitado, se agrega a la lista de visitado y se agrega a la pila para recorrer a sus vecinos
