...
const val TSP_DIR: String = "../tsp" // carpeta con tests TSP
    ...
    private fun algGen(params: Map<String, Any>) {
        ...
        */
        ...
        fun selection(popRanked: MutableList<Individuo>): MutableList<Int> {
            val selectionResults = mutableListOf<Int>()
            val fitnessSum = popRanked.sumOf { it.fitness }
            repeat(2 * popRanked.size) {
                val rand = Random.nextDouble() * fitnessSum
                var cumulativeSum = 0.0
                for ((index, individuo) in popRanked.withIndex()) {
                    cumulativeSum += individuo.fitness
                    if (cumulativeSum >= rand) {
                        selectionResults.add(index)
                        break
                    }
                }
            }
        ...
        fun breed(padre: Individuo, madre: Individuo): Individuo {
            var start = Random.nextInt(padre.tour.size + 1)
            while (start == padre.tour.size) {
                start = Random.nextInt(padre.tour.size + 1)
            }
            var end = Random.nextInt(start, padre.tour.size + 1)
            while (end == start) {
                end = Random.nextInt(start, padre.tour.size + 1)
            }
            var hijoList = padre.tour.subList(start, end).toMutableList()
            var madreList = madre.tour.toMutableList()
            var hijo = MutableList(padre.tour.size) { -1 }
            for (i in start until end) {
                hijo[i] = padre.tour[i]
                if (madreList.contains(padre.tour[i])) {
                    madreList.remove(padre.tour[i])
                }
            }
            for (i in 0 until madreList.size) {
                for (j in 0 until hijo.size) {
                    if (hijo[j] == -1) {
                        hijo[j] = madreList[i]
                        break
                    }
            ...
            return Individuo(tspTest, hijo)
        }

        // Función para mutar un individuo
        fun mutate(individual: Individuo): Individuo {
            var numberRandom = Random.nextDouble(0.0, 1.0)
            if (numberRandom < mutationRate) {
                var index1 = Random.nextInt(individual.tour.size)
                var index2 = Random.nextInt(individual.tour.size)
                var i = individual.tour[index1]
                individual.tour[index1] = individual.tour[index2]
                individual.tour[index2] = i
            }
            return individual
        }
